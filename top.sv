...
`default_nettype none
    ...
    localparam int unsigned DIV = CLK_HZ / 1000;
...
module top #(
    ...
    always_ff @(posedge clk or negedge rst_n) begin
	 ...
	 wire tick;
	 tick_gen_100hz v(.clk(clk), .rst_n(rst), .tick_100hz(tick));

    assign tof_init_start   = KEY_SW[0] & ~key_sw_d[0];
    assign tof_sample_start = tick;
